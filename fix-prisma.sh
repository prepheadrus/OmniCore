#!/bin/bash
# Remove duplicate entries and cleanly add models

# Clean up any bad lines matching our new models if they already exist
sed -i '/model FeedTemplate {/,/}/d' libs/database/prisma/schema.prisma
sed -i '/model ContentRule {/,/}/d' libs/database/prisma/schema.prisma
sed -i '/model FeedQualityRule {/,/}/d' libs/database/prisma/schema.prisma
sed -i '/model AbTest {/,/}/d' libs/database/prisma/schema.prisma

# Find where to add to SalesChannel
