...
function roiLabel(roi: number) { return roi > 40 ? 'Çok Karlı' : roi > 20 ? 'Karlı' : roi > 0 ? 'Düşük Kar' : 'Zarar'; }
    ...
    <div className={cn('min-h-screen bg-slate-50 p-6 transition-all', sidebarOpen ? 'lg:ml-64' : 'ml-16')}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <Card className="shadow-sm"><CardContent className="p-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-50"><DollarSign className="h-4 w-4 text-blue-600" /></div>
          </div>
        ...
        <Card className="shadow-sm"><CardContent className="p-5">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-slate-500 font-medium">Net Kar</p>
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-50"><TrendingUp className="h-4 w-4 text-emerald-600" /></div>
          </div>
        ...
        <Card className="shadow-sm"><CardContent className="p-5">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-slate-500 font-medium">Genel ROI</p>
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-violet-50"><Percent className="h-4 w-4 text-violet-600" /></div>
          </div>
        ...
        <Card className="shadow-sm"><CardContent className="p-5">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-slate-500 font-medium">Ort. Sipariş Değeri</p>
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-amber-50"><ShoppingCart className="h-4 w-4 text-amber-600" /></div>
          </div>
      ...
      <Tabs defaultValue="channels" className="space-y-6">
        <TabsList className="bg-white shadow-sm border">
          ...
          <TabsTrigger value="products" className="gap-2"><Package className="h-4 w-4" />Ürün Karlılığı</TabsTrigger>
        ...
        <TabsContent value="channels">
          <div className="space-y-6">
            {/* Channel Comparison */}
            <Card className="shadow-sm"><CardHeader className="pb-3"><CardTitle className="text-base font-semibold">Kanal Bazlı Gelir Karşılaştırması</CardTitle></CardHeader>
              <CardContent>
                      ...
                      <div key={ch.channel} className="space-y-2">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline" className="text-xs">{ch.orders} sipariş</Badge>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className="text-xs text-slate-400">Gelir: {fmt(ch.revenue)}</span>
                            <span className="text-xs text-slate-400">Komisyon: {fmt(ch.commission)} ({(ch.commissionRate * 100).toFixed(0)}%)</span>
                            <Badge className={cn('text-xs', roiBadge(ch.roi))}>ROI %{ch.roi}</Badge>
                          </div>
                        </div>
                        {/* Revenue bar */}
                        ...
                        <div className="flex gap-4 text-[10px] text-slate-400">
                          <span className="flex items-center gap-1"><span className="h-1.5 w-3 rounded bg-emerald-500" /> Net Kar</span>
                          <span className="flex items-center gap-1"><span className="h-1.5 w-3 rounded bg-amber-400" /> Maliyet</span>
                          <span className="flex items-center gap-1"><span className="h-1.5 w-3 rounded bg-red-300" /> Komisyon</span>
                        </div>
                      </div>
                    );
                  })}
            ...
            <Card className="shadow-sm"><CardHeader className="pb-3"><CardTitle className="text-base font-semibold">Detaylı Kanal Raporu</CardTitle></CardHeader>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-slate-200 bg-slate-50">
                      ...
                      <th className="text-right py-3 px-4 font-medium text-slate-600">Komisyon</th>
                      <th className="text-right py-3 px-4 font-medium text-slate-600">Kargo</th>
                      <th className="text-right py-3 px-4 font-medium text-slate-600">Net Kar</th>
                    ...
                    <tbody>
                      {channelROI.map(ch => (
                        <tr key={ch.channel} className="border-b border-slate-100 hover:bg-slate-50">
                          ...
                          <td className="py-3 px-4 text-right">{fmt(ch.revenue)}</td>
                          ...
                          <td className={cn('py-3 px-4 text-right font-semibold', ch.netProfit >= 0 ? 'text-emerald-600' : 'text-red-600')}>{fmt(ch.netProfit)}</td>
                          <td className={cn('py-3 px-4 text-right font-semibold', roiColor(ch.roi))}>{ch.roi.toFixed(1)}%</td>
                          <td className={cn('py-3 px-4 text-right font-medium', ch.margin >= 0 ? 'text-emerald-600' : 'text-red-600')}>{ch.margin.toFixed(1)}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <TabsContent value="products">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Most Profitable */}
            <Card className="shadow-sm"><CardHeader className="pb-3"><CardTitle className="text-base font-semibold flex items-center gap-2"><ArrowUpRight className="h-4 w-4 text-emerald-500" />En Karlı Ürünler</CardTitle></CardHeader>
              <CardContent className="p-0">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-slate-200 bg-slate-50">
                      <th className="text-left py-2.5 px-3 font-medium text-slate-500 text-xs">Ürün</th>
                      <th className="text-left py-2.5 px-3 font-medium text-slate-500 text-xs">Kanal</th>
                      <th className="text-right py-2.5 px-3 font-medium text-slate-500 text-xs">Net Kar</th>
                      <th className="text-right py-2.5 px-3 font-medium text-slate-500 text-xs">ROI</th>
                    </tr></thead>
                    <tbody>
                      {bestProducts.map(p => (
                        <tr key={p.productId} className="border-b border-slate-100 hover:bg-slate-50">
                          <td className="py-2.5 px-3 font-medium text-slate-800 max-w-[140px] truncate">{p.productName}</td>
                          <td className="py-2.5 px-3"><Badge variant="outline" className="text-xs">{p.channelName}</Badge></td>
                          <td className={cn('py-2.5 px-3 text-right font-semibold', p.netProfit >= 0 ? 'text-emerald-600' : 'text-red-600')}>{fmt(p.netProfit)}</td>
                          <td className="py-2.5 px-3 text-right"><Badge className={cn('text-xs', roiBadge(p.roi))}>{p.roi.toFixed(1)}%</Badge></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {/* Least Profitable */}
            <Card className="shadow-sm"><CardHeader className="pb-3"><CardTitle className="text-base font-semibold flex items-center gap-2"><ArrowDownRight className="h-4 w-4 text-red-500" />En Az Karlı Ürünler</CardTitle></CardHeader>
              <CardContent className="p-0">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-slate-200 bg-slate-50">
                    ...
                    <div key={i} className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3 text-xs text-slate-500">
                          ...
                          <span>{m.orders} sipariş</span>
                        </div>
                      </div>
                      {/* Revenue bar */}
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-slate-400 w-10 text-right">Gelir</span>
                      ...
                      {/* Profit bar */}
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-slate-400 w-10 text-right">Net</span>
                        <div className="flex-1 h-4 bg-slate-100 rounded-full overflow-hidden">
                          <div className={cn('h-full rounded-full transition-all duration-500', m.netProfit >= 0 ? 'bg-emerald-500' : 'bg-red-500')} style={{ width: `${netPct}%` }} />
                        </div>
                      </div>
                    </div>
                  );
