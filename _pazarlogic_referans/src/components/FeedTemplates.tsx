...
import {
  Search, Download, Star, Flame, ChevronRight, ChevronLeft, Plus, Trash2,
    ...
    <div className={`${sidebarOpen ? 'lg:ml-64' : 'ml-16'} min-h-screen bg-slate-50 p-6 transition-all`}>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Pazaryeri feed şablonlarını yönetin, yeni feed oluşturun ve kalite kontrollerini çalıştırın
        </p>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 bg-white rounded-lg shadow-sm p-1 mb-6 border border-slate-200">
        {tabs.map((tab) => (
          <button
            key={tab.key}
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={stat.label} className="bg-white rounded-lg shadow-sm border border-slate-200 p-4 flex items-center gap-3">
            <div className="h-10 w-10 rounded-lg bg-slate-50 flex items-center justify-center">{stat.icon}</div>
            <div>
        ...
        <div className="flex flex-col lg:flex-row gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <select
              ...
              className="px-3 py-2 rounded-lg border border-slate-200 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-white"
            >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-6">
          {templates.map((t) => (
            <div key={t.id} className="bg-white rounded-lg shadow-sm border border-slate-200 p-4 hover:shadow-md transition-shadow flex flex-col">
              {/* Top row: platform + popular */}
              <div className="flex items-start justify-between mb-3">
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${platformColors[t.platform] || 'bg-slate-100 text-slate-700 border-slate-200'}`}>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {t.category && (
                  <span className="px-2 py-0.5 rounded-md bg-slate-100 text-slate-600 text-[11px] font-medium">{t.category}</span>
                )}
                <span className={`px-2 py-0.5 rounded-md text-[11px] font-bold uppercase ${formatColors[t.format] || 'bg-slate-100 text-slate-600'}`}>
                  {t.format}
                </span>
              </div>
...
function FeedCreator() {
  ...
  // Compute default field mappings when entering step 3
          ...
          {wizardSteps.map((s, i) => (
            <div key={s.num} className="flex items-center flex-1">
              ...
              {i < wizardSteps.length - 1 && (
                <div className={`flex-1 h-0.5 mx-3 mt-[-16px] ${step > s.num ? 'bg-emerald-500' : 'bg-slate-200'}`} />
              )}
            </div>
          ))}
        </div>
            ...
            {allPlatforms.map((p) => (
              ...
              >
                <div className={`h-12 w-12 rounded-xl flex items-center justify-center text-lg font-black ${
                  selectedPlatform === p ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-500'
                }`}>
                  {platformIcons[p]}
                </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
            ...
            {templates.slice(0, 6).map((t) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${formatColors[t.format] || 'bg-slate-100 text-slate-600'}`}>
                    {t.format}
                  </span>
                  <StarRating rating={t.rating} />
                </div>
          ...
          <div className="flex justify-between mt-6">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-600 text-sm font-medium transition-colors"
            >
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {targetFields.map((target, i) => {
                  ...
                  return (
                    <tr key={target} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-2.5 px-4">
                        <select
                          ...
                          className="w-full px-3 py-1.5 rounded-md border border-slate-200 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-white"
                        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
            <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
              ...
              <p className="text-sm font-semibold text-slate-700 flex items-center gap-2 mt-1">
                <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${platformColors[selectedPlatform] || 'bg-slate-100 text-slate-600'}`}>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Toplam Kural', value: summary.total, color: 'text-slate-700', bg: 'bg-slate-50', icon: <Settings className="h-5 w-5 text-slate-400" /> },
          ...
          <div key={s.label} className={`${s.bg} rounded-lg border border-slate-200 p-4 flex items-center gap-3`}>
            {s.icon}
            <div>
              ...
              <tbody>
                {checkResults.map((r, i) => (
                  <tr key={i} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-2.5 px-4">
                      <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-600 text-xs font-medium">{r.field}</span>
                    </td>
                    ...
                    <td className="py-2.5 px-4">
                      <span className={`px-2 py-0.5 rounded text-[11px] font-medium ${platformColors[r.channel] || 'bg-slate-100 text-slate-600'}`}>
                        {r.channel}
                      </span>
                    </td>
                  </tr>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-100 max-h-[500px] overflow-y-auto">
            {rules.map((rule) => (
              <div key={rule.id} className="flex items-center gap-4 p-4 hover:bg-slate-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {rule.isActive ? (
                      ...
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-500">PASİF</span>
                    )}
                  </div>
                  <p className="text-xs text-slate-500 mt-0.5">
